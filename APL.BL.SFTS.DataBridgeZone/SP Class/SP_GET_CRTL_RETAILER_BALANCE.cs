...
namespace APL.BL.SFTS.DataBridgeZone.SP_Class
{
    [Serializable()]
    [DataContract]
    public class SP_GET_CRTL_RETAILER_BALANCE : ObjectMakerFromOracleSP.ISpClassEntity
    {
        [DataMember]
        ...
        {
            SP_GET_CRTL_RETAILER_BALANCE entity = new SP_GET_CRTL_RETAILER_BALANCE();
            ...
            if (values[13].GetType() != typeof(System.DBNull))
                entity.IS_ACTIVE_STR = Convert.ToDecimal(values[13])==1?"Active":"Inactive";

            if (values[14].GetType() != typeof(System.DBNull))
                entity.RETAILER_NAME = Convert.ToString(values[14]);

            if (values[15].GetType() != typeof(System.DBNull))
                entity.DISTRIBUTOR_NAME = Convert.ToString(values[15]);
