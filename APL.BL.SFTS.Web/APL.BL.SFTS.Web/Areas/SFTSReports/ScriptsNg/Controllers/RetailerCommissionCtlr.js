/**
 * RetailerCommissionCtlr.js
 */

app.controller('RetailerCommissionCtlr', ['$scope', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, crudService, conversion, $filter, $localStorage, uiGridConstants) {

        var baseUrl = DirectoryKey +'/SFTSReports/api/Retailer/';
      
        $scope.PageTitle = 'Retailer Commission';
        $scope.IsClusterDisabled = true;
        $scope.IsRegionDisabled = true;
        $scope.IsZoneDisabled = true;
        $scope.IsRouteDisabled = true;
        $scope.gridOptionsRetailerCommission = [];
        ...
        $scope.GetRetailerCommission = function () {
            $scope.gridOptionsRetailerCommission.enableFiltering = false;
            $scope.gridOptionsRetailerCommission.enableGridMenu = true;

            //For Loading
            $scope.loaderMoreRetailerCommission = true;
            $scope.lblMessageForRetailerCommission = 'loading please wait....!';
            ...
            $scope.gridOptionsRetailerCommission = {
                ...
                columnDefs: [
                    ...
                    { name: "RETAILER_CODE", displayName: "Retailer Code", title: "Retailer Code", width: '14%', headerCellClass: $scope.highlightFilteredHeader },
                    ...
                    { name: "DISBURSE_DATE", displayName: "Disburse Date", title: "Disburse Date", width: '12%', headerCellClass: $scope.highlightFilteredHeader },
                ...
                enableGridMenu: true,
                enableSelectAll: true,
                exporterCsvFilename: 'Retailer Commission.csv',
                exporterPdfDefaultStyle: { fontSize: 9 },
                exporterPdfTableStyle: { margin: [30, 30, 30, 30] },
                exporterPdfTableHeaderStyle: { fontSize: 10, bold: true, italics: true, color: 'red' },
                exporterPdfHeader: { text: "Retailer Commission", style: 'headerStyle' },
            ...
            var apiRoute = baseUrl + 'GetRetailerCommission/';
            ...
            tva.then(function (response) {
                $scope.tvaList = response.data.retailerCommission;
                $scope.gridOptionsRetailerCommission.data = response.data.retailerCommission;
                $scope.loaderMoreRetailerCommission = false;
            },
            function (error) {
                console.log("Error: " + error);
            });
