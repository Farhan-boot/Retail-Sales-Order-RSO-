...
app.controller('NewGraphInfoCtrl', ['$scope', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        var baseUrl = DirectoryKey +'/VFocusAPI/api/denomapping/';
        var isExisting = 0;
        var page = 0;
        var pageSize = 100;
        var isPaging = 0;
        ...
        $scope.PageTitle = 'Deno Mapping';
        $scope.ListTitle = 'Deno Mapping List';
        ...
        */
        ...
        function loadUNASSINGGRPH(i) {
           // alert('hello'); debugger;
            ...
            objcmnParam = {
                ...
                IsPaging: isPaging,
                RoleId: 0,
                LASTUPDATEBY: $scope.loggedUserId,
                KPIID: i,
            };

            var apiRoute = DirectoryKey + '/VFocusAPI/api/vFocusDropdown/GetAllAssignUnAssignDENO/';
            var cmnParam = "[" + JSON.stringify(objcmnParam) + "]";

            crudService.GetList(apiRoute, cmnParam, $scope.HeaderToken)
                .then(function (response) {
                    $scope.unassignGraph = response.data.objListUNASSDENO;
                    $scope.assignGraph = [];
                },
                    function (error) {
                        console.error(error)
        ...
        function loadAllNewGraphInfo(isPaging) {
            ...
            $scope.gridOptionsNewGraphInfo = {
                ...
                columnDefs: [                    
                    { name: "PACK_NAME", displayName: "PACK_NAME", title: "PACK_NAME", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "RECH_DENO", displayName: "RECH_DENO", title: "RECH_DENO", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "RECH_TYPE", displayName: "RECH_TYPE", title: "RECH_TYPE", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            var apiRoute = baseUrl + 'GetAllDenomap/';
            ...
            listCurrentOfferInfo.then(function (response) {
                ...
                $scope.gridOptionsNewGraphInfo.data = response.data.DENOList;
                $scope.loaderMoreNewGraphInfo = false;
                $scope.currentOfferList = response.data.DENOList;
            },
                function (error) {
                    console.log("Error: " + error);
                });
        ...
        $scope.SaveNewSurvey = function () {
            ...
            var GraphInfo = {
                ...
                // Detail: $scope.Detail,
                LASTUPDATEBY: $scope.loggedUserId,
                AssingDENO: $scope.assignGraph,
                UnassingDENO: $scope.unassignGraph

            };

            // var HedarTokenPostPut = $scope.CustomerID > 0 ? $scope.HeaderToken.put : $scope.HeaderToken.post;
            var apiRoute = baseUrl + 'SaveDenomap/';
