    ...
    function ($scope, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        $scope.loadColorRecords = function (isPaging) {
            ...
            $scope.gridOptionsColor = {
                columnDefs: [
                    ...
                    { name: "DyingColorName", title: "Dying Color", headerCellClass: $scope.highlightFilteredHeader },
                    ...
                    { name: "ColorCode", title: "Color Code", headerCellClass: $scope.highlightFilteredHeader },
                    {
                        ...
                        cellTemplate:
                                        ...
                                        //    '</a>' +
                                        //'</span>' +
                                        '<span class="label label-success label-mini">' +// ng-if="grid.appScope.UserCommonEntity.EnableUpdate"
        ...
        $scope.getColor = function (dataModel) {
            ...
            singleColor.then(function (response) {
                ...
                $scope.drpDyningColor = response.data.objColor.DyingItemColorID;
               

                if (response.data.objColor.DyingColorName != 'N/A') {
                    $("#drpDyningColor").select2("data", { id: response.data.objColor.DyingItemColorID, text: response.data.objColor.DyingColorName });
                } else {

                    $("#drpDyningColor").select2("data", { id: 0, text: '--Dying Color--' });
                }
        ...
        $scope.save = function () {
            ...
            var Color = {
                ...
                DyingItemColorID: $scope.drpDyningColor
            };
        ...
        $scope.loadDelModel = function (EntityModel) {
            ...
            $scope.UserCommonEntity.DelMsgs = "You are about to delete " + EntityModel.ColorName + " color.";
        }
        ...
        $scope.GetDyingColors = function () {
            var apiRoute = baseUrl + 'GetDyingColors/';
            var dyningColor = crudService.getAllWithCompnayID(apiRoute, page, pageSize, isPaging, LoggedCompanyID);
            dyningColor.then(function (response) {            
                $scope.DyningColors = response.data;
            },
            function (error) {
                console.log("Error: " + error);
            });
        }
        $scope.GetDyingColors();
