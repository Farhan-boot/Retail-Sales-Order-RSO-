...
app.controller('TargetSetupCtlr', ['$scope', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        var baseUrl = DirectoryKey +'/SFTS/api/TargetSetup/';
        var isExisting = 0;
        var page = 0;
        var pageSize = 100;
        var isPaging = 0;
        ...
        function SaveUploadedExcel() {
            ...
            // ADD LISTENERS.
            var apiRoute = baseUrl + 'SaveUpdateTargetExcel/';
        ...
        function getSavedExcelData() {
            ...
            upCode.then(function (response) {
                ...
                if ($scope.uploadedData.length > 0) {
                    ...
                    } else if ($scope.uploadedData[0].APPLY_TYPE == 2) {
                        $scope.uploadedStaffExcelData = response.data.FileDataList;
                        $scope.TotalRowCount = 0; // $scope.uploadedStaffExcelData[0].ROW_COUNT;
                        $scope.TotalTargetValue = 0; // $scope.uploadedStaffExcelData[0].TOTAL_TARGET_VALUE;
                        //if ($scope.uploadedStaffExcelData != null || $scope.uploadedStaffExcelData != [] || $scope.uploadedStaffExcelData != "" || $scope.uploadedStaffExcelData != undefined) {
                        if ($scope.uploadedStaffExcelData != null ) {
                            $scope.TotalRowCount = $scope.uploadedStaffExcelData[0].ROW_COUNT;
                            $scope.TotalTargetValue = $scope.uploadedStaffExcelData[0].TOTAL_TARGET_VALUE;

                            $scope.IsStaffExcelDataShow = true;
                            $scope.IsExcelDataShow = false;
                            $scope.IsInProgress = true;
                            $scope.ProgressStatus = 100;
                            $scope.waitText = ' Completed!'

                            if ($scope.uploadedData[0].APPLY_SUB_TYPE_ID == 1) {
                                $scope.IsRSOShow = true;
                                $scope.IsMonoBrandShow = false;
                            } else if ($scope.uploadedData[0].APPLY_SUB_TYPE_ID == 2) {
                                $scope.IsRSOShow = false;
                                $scope.IsMonoBrandShow = true;
                            }
                        }
                    }
                }
        ...
        $scope.ExportExcelFile = function () {
           // alert('Hello'); debugger;
            fileExport = {
                TargetItemId: ($scope.TargetItem == null) ? 0 : $scope.TargetItem,
                TargetPeriodId: ($scope.TargetPeriod == null) ? 0 : $scope.TargetPeriod,
                TargetFor: ($scope.TargetFor == null) ? 0 : $scope.TargetFor,
                StaffTypeId: ($scope.StaffType == null) ? 0 : $scope.StaffType
                //loggeduser: $scope.loggedUserId,                          
            };

            var apiRoute = baseUrl + 'ExportTargetExcel/';
            ...
            TargetItems.then(function (response) {
                window.open('/mDMS_NEW/UserFiles/TargetExcel/' + response.data, '_blank', '');
            },
            function (error) {
                console.log("Error: " + error);
            });
        ...
        function loadAllTarget(isPaging) {
            ...
            $scope.gridOptionsTargetSetup = {
                ...
                columnDefs: [
                    ...
                    { name: "PERIOD_NAME", displayName: "Period", title: "Period", width: '16%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "SET_DATE", displayName: "Set Date", title: "Set Date", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "VERSION", displayName: "Version", title: "Version", width: '7%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "TARGET_FOR", displayName: "Target For", title: "Target For", width: '17%', headerCellClass: $scope.highlightFilteredHeader },
                    {
                        name: 'Action',
                        displayName: "Action",
                ...
                enableGridMenu: true,
                enableSelectAll: true,
                exporterCsvFilename: 'TargetSetup.csv',
            ...
            var apiRoute = baseUrl + 'GetAllTarget/';
        ...
        $scope.SaveTargetSetup = function () {
            ...
            var apiRoute = baseUrl + 'SaveTarget/';
