 ...
 * CurrentOfferCtrl.js
 */
app.controller('NewSurveyCtlr', ['$scope', 'crudService', 'conversion', '$filter', '$localStorage', 'uiGridConstants',
    function ($scope, crudService, conversion, $filter, $localStorage, uiGridConstants) {
        ...
        //**********----Get Single Record----***************
                $scope.getNewSurveyInfo = function (dataModel) {
                    ...
                    $scope.Distributor = dataModel.DISTRIBUTORID == null ? '' : dataModel.DISTRIBUTORID;
                    $scope.Route = dataModel.ROUTE_CODE;
                    $scope.ROUTEID = dataModel.ROUTEID;
                    ...
                    $scope.loadRoutes();
                   // alert(dataModel.ROUTE_NAME + ', ' + dataModel.ROUTEID  )
                    $("#ddlDistributor").select2("data", { id: dataModel.DISTRIBUTORID, text: dataModel.DISTRIBUTOR_NAME??'--Select Distributor--' });
                    //$("#ddlRoute").select2("val", { id: dataModel.ROUTEID});
                    $("#ddlRoute").select2("data", { id: dataModel.ROUTEID, text: dataModel.ROUTE_NAME??'--Select Route--' });
        ...
        $scope.save = function () {
           // $('#save').attr('style', 'pointer-events: none;');
            ...
            if ($scope.SURVEY_FOR == " ") {
                alert('Sorry! survey for not selected');
                ...
                $scope.SaveNewSurvey = function () {
                    ...
                    var SurveyInfo = {
                        ...
                        SURVEY_FOR: $scope.SURVEY_FOR == "" ? "0" : $scope.SURVEY_FOR
                    };
                    ...
                    var cmnParam = "[" + JSON.stringify(objcmnParam) + "," + JSON.stringify(SurveyInfo) + "," + JSON.stringify($scope.ListSelectedRegion) + "]";
